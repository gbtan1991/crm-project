...
export class GoogleCalendarOAuth {
  private static readonly CLIENT_ID = env.GOOGLE_CLIENT_ID;
  private static readonly CLIENT_SECRET = env.GOOGLE_CLIENT_SECRET;
  private static readonly REDIRECT_URI = `${env.NEXT_PUBLIC_URL}/api/google-calendar/callback`;
  ...
  static async exchangeCodeForToken(code: string): Promise<{
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Google Calendar OAuth error: ${error.error_description ?? "Unknown error"}`,
      );
    }
  ...
  static async getPrimaryCalendarTimeZone(
    businessId: string,
  ): Promise<string | null> {
